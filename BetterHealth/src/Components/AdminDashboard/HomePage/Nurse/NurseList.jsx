...
const NurseList = () => {
    ...
    useEffect(() => {
        const fetchNurse = async () => {
            try{
                const res = await axios.get('https://hms-w4kw.onrender.com/api/Nurse/GetAllNurses')
                setNurses(res.data.data.$values)
                        ...
                        return(
                            <div key={nurse.nurseId} className='w-[250px] overflow-hidden shadow-2xl border border-stone-100 rounded-lg bg-white'
                                ...
                                <div className='text-2xl text-white font-medium bg-primary2 flex flex-col items-center p-2 '>
                                    <p className=''>{nurse.firstname} {nurse.lastname}</p>
                                    <p className='font-medium text-sm text-gray-300'>{nurse.specialization}</p>
                                </div>
                            </div>
                        )
                    })}
